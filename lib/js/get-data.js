  ...
  clientProm.then(client => {
    ...
    client.request("Observation?" + query, {
      ...
      function (ob) {
        ...
        heights = dedupAndPair(heights);
        weights = dedupAndPair(weights);
        ...
        hdl = dedupAndPair(hdl);
        ldl = dedupAndPair(ldl);
        systolicbps = dedupAndPair(systolicbps);
        diastolicbps = dedupAndPair(diastolicbps);
        bmis = dedupAndPair(bmis);
        ...
        heights = { name: "height", data: heights };
        weights = { name: "weight", data: weights };

        // add randomness for the sake of graphical testing
        // heights.data = heights.data.map(x=> {x[1]+= Math.round(Math.random()*10)-5; return x;})
        // weights.data = weights.data.map(x=> {x[1] += Math.round(Math.random()*10)-5; return x;})

        hdl = { name: "hdl", data: hdl };
        ldl = { name: "ldl", data: ldl };
        systolicbps = { name: "systolic", data: systolicbps };
        diastolicbps = { name: "diastolic", data: diastolicbps };
        bmis = { name: "bmi", data: bmis };
        // bmi.data = bmi.data.map(x=> {x[1] += Math.round(Math.random()*10)-5; return x;})
