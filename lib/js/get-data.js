...
function populatePatient() {
  ...
  clientProm.then(client => {
    ...
    // TODO check incoming value against high bands to determine trigger
    client.request("Observation?" + query, {
      ...
      function (ob) {
        ...
        if (hdl.filter(item => dateFromEffective(item.effectiveDateTime) > startDate)
          .some(item => getQuantityValue(item) <= 40))
          vm.high_values += "hdl cholesterol"
        if (ldl.filter(item => dateFromEffective(item.effectiveDateTime) > startDate)
          .some(item => getQuantityValue(item) >= 100))
          vm.high_values += "ldl cholesterol"
        if (systolicbp.filter(item => dateFromEffective(item.effectiveDateTime) > startDate)
          .some(item => getQuantityValue(item) >= 130))
          vm.high_values += "systolic bp"
        if (diastolicbp.filter(item => dateFromEffective(item.effectiveDateTime) > startDate)
          .some(item => getQuantityValue(item) >= 90))
          vm.high_values += "diastolic bp"
        if (bmi.filter(item => dateFromEffective(item.effectiveDateTime) > startDate)
          .some(item => getQuantityValue(item) >= 25 || getQuantityValue(item) <= 18.5))
          vm.high_values += "body mass index"
    ...
    function addLDL() {
      ...
      if (value >= 100) {
        vm.displayModal("High HDL Cholesterol", "Looks like your LDL (bad) Cholesterol is a bit high. Check with your doctor and look at the provided resources for tips on managing your cholesterol.")
      }
    ...
    function addBP() {
      ...
      if (sys_value === "" || sys_value === null || sys_value === undefined || isNaN(parseFloat(sys_value)) ||
        dia_value === "" || dia_value === null || dia_value === undefined || isNaN(parseFloat(dia_value))) {
        vm.displayModal('Invalid input', 'value must be filled out and numeric');
        return;
      }

      // check for high values
      if (dia_value >= 80 || sys_value >= 120) {
